module History
  ...
  class Container
    ...
    def store(session, request, response)
      ...
      # don't store puts and deletes
      # if request.post? or request.put? or request.delete?
      if request.put? or request.delete? 
    ...
    def force_store(session, request, response)
      ...
      # don't store refresh
      if request.request_uri.clean_uri == session[:history].last
        logger.debug("history: skipped refresh request")
        return
      end

      session[:history].push(request.request_uri.clean_uri)
      while session[:history].size > @max do
        session[:history].shift
      end
      logger.debug("history: added #{request.request_uri.clean_uri}")
    end
